...
&НаКлиенте
Процедура ТаблицаСостояниеПередНачаломДобавления(Элемент, Отказ, Копирование, Родитель, Группа, Параметр)
...
&НаКлиенте
Процедура ТаблицаСостояниеПередУдалением(Элемент, Отказ)
...
&НаКлиенте
Процедура ТаблицаСостояниеВыбор(Элемент, ВыбраннаяСтрока, Поле, СтандартнаяОбработка)
	
	Если Поле.Имя = "ТаблицаСостояниеСотрудник"	
		Или Поле.Имя = "ТаблицаСостояниеОрганизация" Тогда
		...
		Если Поле.Имя = "ТаблицаСостояниеСотрудник" Тогда
			Значение = ТекущиеДанные.Сотрудник;	
		ИначеЕсли Поле.Имя = "ТаблицаСостояниеОрганизация" Тогда
			Значение = ТекущиеДанные.Организация;	
		Иначе
...
Функция _ОтправитьФайлыСотруднику(Сотрудник, УникальныйИдентификатор) Экспорт
	...
	Попытка
		...
		Если НЕ КИ.Количество() Тогда
			ВызватьИсключение "Не заполнен email сотрудника";	
		КонецЕсли;
