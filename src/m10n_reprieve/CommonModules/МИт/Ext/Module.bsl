...
Функция ОписаниеПодтвержденияПоСотруднику(Сотрудник) Экспорт
	...
	Настройки = НастройкиОписанияПодтверждения(Сотрудник.ГоловнаяОрганизация);
	Обработки.МИтХранилище.Создать().ЗаполнитьОписаниеПодтверждения(ОписаниеПодтверждения, Сотрудник, Настройки);
...
Процедура УстановитьИспользоватьСвойства() Экспорт
...
Процедура ЗаписатьЗначениеСвойства(Объект, Свойство, Значение) Экспорт
	...
	Если НЕ МИтСеансПовтИсп.ИспользоватьДополнительныеРеквизитыИСведения() Тогда
		УстановитьИспользоватьСвойства();	
	КонецЕсли;
...
Функция НастройкиОписанияПодтверждения(Организация) Экспорт

	Результат = Новый Структура;
	
	ГенДирФИО = ЗначениеСвойства(Организация, МИтКС.СвойствоФиоДиректора());
	ГенДирСНИЛС = ЗначениеСвойства(Организация, МИтКС.СвойствоСнилсДиректора());
	МобТелефонГенДирИлиИспОрган = ЗначениеСвойства(Организация, МИтКС.СвойствоТелефонДиректора());
	МобТелефонОтветсвенногоКадры = ЗначениеСвойства(Организация, МИтКС.СвойствоТелефонКадровика());
	
	Если ЗначениеЗаполнено(ГенДирФИО) Тогда
		Результат.Вставить("ГенДирФИО", ГенДирФИО);
	КонецЕсли;
	
	Если ЗначениеЗаполнено(ГенДирСНИЛС) Тогда
		Результат.Вставить("ГенДирСНИЛС", ГенДирСНИЛС);
	КонецЕсли;
	
	Если ЗначениеЗаполнено(МобТелефонГенДирИлиИспОрган) Тогда
		Результат.Вставить("МобТелефонГенДирИлиИспОрган", МобТелефонГенДирИлиИспОрган);
	КонецЕсли;
	
	Если ЗначениеЗаполнено(МобТелефонОтветсвенногоКадры) Тогда
		Результат.Вставить("МобТелефонОтветсвенногоКадры", МобТелефонОтветсвенногоКадры);
	КонецЕсли;
